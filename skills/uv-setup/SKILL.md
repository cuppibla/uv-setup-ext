...
name: uv-setup
description: >
  Use this skill when the user gets a "command not found" error for a Python
  tool, wants to set up uv for any Python project, needs to install a package
  dependency, or asks how to run a Python command correctly. Also activate when
  the user is starting a new Python project and needs a proper environment.
---
...
After `uv init` + `uv add <package>`, the pyproject.toml should look like:
...
uv sync                 # sync environment after manually editing pyproject.toml
